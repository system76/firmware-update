...
impl ConfigurationTable {
    pub fn kind(&self) -> ConfigurationKind {
        match self.VendorGuid {
            NULL_GUID => ConfigurationKind::Null,
            MPS_TABLE_GUID => ConfigurationKind::Mps,
            ACPI_TABLE_GUID => ConfigurationKind::Acpi,
            ACPI_20_TABLE_GUID => ConfigurationKind::Acpi2,
            SMBIOS_TABLE_GUID => ConfigurationKind::Smbios,
            SMBIOS3_TABLE_GUID => ConfigurationKind::Smbios3,
            SAL_SYSTEM_TABLE_GUID => ConfigurationKind::SalSystem,
            HCDP_TABLE_GUID => ConfigurationKind::Hcdp,
            UGA_IO_PROTOCOL_GUID => ConfigurationKind::UgaIo,
            EFI_GLOBAL_VARIABLE_GUID => ConfigurationKind::GlobalVariable,
            UV_SYSTEM_TABLE_GUID => ConfigurationKind::UvSystem,
            LINUX_EFI_CRASH_GUID => ConfigurationKind::LinuxEfiCrash,
            LOADED_IMAGE_PROTOCOL_GUID => ConfigurationKind::LoadedImage,
            EFI_GRAPHICS_OUTPUT_PROTOCOL_GUID => ConfigurationKind::GraphicsOutput,
            EFI_UGA_PROTOCOL_GUID => ConfigurationKind::Uga,
            EFI_PCI_IO_PROTOCOL_GUID => ConfigurationKind::PciIo,
            EFI_FILE_INFO_ID => ConfigurationKind::FileInfo,
            EFI_SYSTEM_RESOURCE_TABLE_GUID => ConfigurationKind::SystemResource,
            EFI_FILE_SYSTEM_GUID => ConfigurationKind::FileSystem,
            DEVICE_TREE_GUID => ConfigurationKind::DeviceTree,
            EFI_PROPERTIES_TABLE_GUID => ConfigurationKind::Properties,
            EFI_RNG_PROTOCOL_GUID => ConfigurationKind::Rng,
            EFI_RNG_ALGORITHM_RAW => ConfigurationKind::RngAlrorithm,
            EFI_MEMORY_ATTRIBUTES_TABLE_GUID => ConfigurationKind::MemoryAttributes,
            EFI_CONSOLE_OUT_DEVICE_GUID => ConfigurationKind::ConsoleOut,
            EFI_SECTION_TIANO_COMPRESS_GUID => ConfigurationKind::SectionTianoCompress,
            EFI_SECTION_LZMA_COMPRESS_GUID => ConfigurationKind::SectionLzmaCompress,
            EFI_DXE_SERVICES_TABLE_GUID => ConfigurationKind::DxeServices,
            EFI_HOB_LIST_GUID => ConfigurationKind::HobList,
            MEMORY_TYPE_INFORMATION_GUID => ConfigurationKind::MemoryTypeInformation,
            EFI_DEBUG_IMAGE_INFO_TABLE_GUID => ConfigurationKind::DebugImageInfo,
            _ => ConfigurationKind::Unknown
        }
    }
}
